...
class SentryPlugin extends PluginBase {
  ...
  async init(pluginConfig, callback) {
    ...
    if (!pluginConfig.dsn) {
      return (
        ...
        callback(
          new Error(
            "Invalid Sentry definition, no dsn provided. Disable error reporting"
          ),
          false
        )
      );
    ...
    if (process.env.TRAVIS || process.env.APPVEYOR || process.env.CI) {
      this.log.info(
        "Sentry Plugin disabled for this process because CI system detected"
      );
    ...
    if (
      ...
      this.parentIoPackage.common.disableDataReporting
    ) {
      this.log.info(
        "Sentry Plugin disabled for this process because data reporting is disabled on instance"
      );
      return callback && callback(null, true);
    }
    ...
    if (
      ...
      let hostObj;
      try {
        hostObj = await this.getObjectAsync(
      ...
      if (!hostObjName) {
        const posPluginInNamespace =
          this.pluginNamespace.indexOf(".plugins.sentry");
  ...
  _registerSentry(pluginConfig, uuid, callback) {
    ...
    if (
      pluginConfig.pathWhitelist &&
      Array.isArray(pluginConfig.pathWhitelist)
    ) {
      sentryPathWhitelist = pluginConfig.pathWhitelist;
    }
    if (
      this.parentPackage.name &&
      !sentryPathWhitelist.includes(this.parentPackage.name)
    ) {
      sentryPathWhitelist.push(this.parentPackage.name);
    }
    let sentryPathBlacklist = [];
    if (
      pluginConfig.pathBlacklist &&
      Array.isArray(pluginConfig.pathBlacklist)
    ) {
      sentryPathBlacklist = pluginConfig.pathBlacklist;
    }
    let sentryErrorBlacklist = [];
    if (
      pluginConfig.errorBlacklist &&
      Array.isArray(pluginConfig.errorBlacklist)
    ) {
    ...
    this.Sentry.configureScope((scope) => {
      if (this.parentIoPackage && this.parentIoPackage.common) {
        scope.setTag(
          "version",
          this.parentIoPackage.common.installedVersion ||
            this.parentIoPackage.common.version
        );
        if (this.parentIoPackage.common.installedFrom) {
          scope.setTag(
            "installedFrom",
            this.parentIoPackage.common.installedFrom
          );
        } else {
          scope.setTag(
            "installedFrom",
      ...
      scope.addEventProcessor((event, hint) => {
        ...
        if (
          ...
          if (
            ...
            if (
              eventData.stacktrace.frames[
                eventData.stacktrace.frames.length - 1
              ].filename &&
              (eventData.stacktrace.frames[
                eventData.stacktrace.frames.length - 1
            ...
            const whitelisted = eventData.stacktrace.frames.find((frame) => {
              ...
              if (
                frame.filename &&
                !sentryPathWhitelist.find(
                  (path) => path && path.length && frame.filename.includes(path)
                )
              ) {
                return false;
              }
